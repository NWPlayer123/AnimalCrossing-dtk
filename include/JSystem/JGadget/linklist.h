...
template <typename T> class TPRIsEqual_pointer_ {
  public:
    TPRIsEqual_pointer_<T>(const T* p) {
        this->p_ = p;
    }
  ...
  public:
    ...
    class iterator {
      public:
        ...
        friend bool operator==(TNodeLinkList::iterator lhs, TNodeLinkList::iterator rhs) {
    ...
    class const_iterator {
      public:
        ...
        const_iterator(const TLinkListNode* node) {
            this->p_ = node;
        }
        ...
        friend bool operator==(TNodeLinkList::const_iterator lhs, TNodeLinkList::const_iterator rhs) {
            return lhs.p_ == rhs.p_;
        }
        friend bool operator!=(TNodeLinkList::const_iterator lhs, TNodeLinkList::const_iterator rhs) {
            return !(lhs == rhs);
...
template <typename T, int O> class TLinkList : public TNodeLinkList {
  public:
    class iterator {
      public:
        ...
        T* operator->() const {
            return TLinkList::Element_toValue(mIt.operator->());
        }
        T& operator*() const {
            T* p = this->operator->();
#line 541
...
#line 586
    ...
    iterator begin() {
        return TNodeLinkList::begin();
    }
    const_iterator begin() const {
        return const_cast<TLinkList*>(this)->begin();
    }

    iterator end() {
        return TNodeLinkList::end();
    }
    const_iterator end() const {
        return const_cast<TLinkList*>(this)->end();
    }

    iterator Find(const T* p) {
        return TNodeLinkList::Find(TLinkList::Element_toNode(p));
    }
    iterator Erase(T* p) {
        return TNodeLinkList::Erase(Element_toNode(p));
    }
    iterator Insert(iterator it, T* p) {
        return TNodeLinkList::Insert(it.mIt, TLinkList::Element_toNode(p));
    }
    void Remove(T* p) {
        TNodeLinkList::Remove(TLinkList::Element_toNode(p));
    }
    void Push_front(T* p) {
        Insert(begin(), p);
    }
