...
static int aKonAtqclock_DwAfter(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                void* arg, s_xyz* joint1, xyz_t* trans) {
    return 1;
}

static int aKonAtqclock_DwBefore(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                 void* arg, s_xyz* joint1, xyz_t* trans) {
...
static void aKonAtqclock_dw(FTR_ACTOR* ftr_actor, ACTOR* my_room_actor, GAME* game, u8* data) {
    cKF_Si3_draw_R_SV(game, &ftr_actor->keyframe, ftr_actor->skeleton_mtx[game->frame_counter & 1],
                      aKonAtqclock_DwBefore, aKonAtqclock_DwAfter, ftr_actor);
}
...
static void aKonAtqclock_ct(FTR_ACTOR* ftr_actor, u8* data) {
    ...
    cKF_SkeletonInfo_R_ct(keyf, &cKF_bs_r_int_kon_atqclock, &cKF_ba_r_int_kon_atqclock, ftr_actor->joint,
                          ftr_actor->morph);
...
static aFTR_vtable_c aKonAtqclock_func = {
    &aKonAtqclock_ct, &aKonAtqclock_mv, &aKonAtqclock_dw, &aKonAtqclock_dt, NULL,
};
