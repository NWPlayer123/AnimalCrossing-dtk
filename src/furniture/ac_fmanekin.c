...
static void aFmanekin_dma(mActor_name_t ftr_name, u8* data) {
    mPlib_Load_PlayerTexAndPallet(
        data, data + 0x200,
        ((ftr_name >= FTR_CLOTH_START) && (ftr_name <= FTR_CLOTH_END)) ? (ftr_name - FTR_CLOTH_START) >> 2 : 0);
}

static aFTR_vtable_c aFmanekin_func = {
    &aFmanekin_ct, &aFmanekin_mv, &aFmanekin_dw, &aFmanekin_dt, &aFmanekin_dma,
};
