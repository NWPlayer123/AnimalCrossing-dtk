...
static int aNogTriClock01_DwAfter(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                  void* arg, s_xyz* joint1, xyz_t* trans) {
    return 1;
}

static int aNogTriClock01_DwBefore(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                   void* arg, s_xyz* joint1, xyz_t* trans) {
...
static void aNogTriClock01_dw(FTR_ACTOR* ftr_actor, ACTOR* my_room_actor, GAME* game, u8* data) {
    cKF_Si3_draw_R_SV(game, &ftr_actor->keyframe, ftr_actor->skeleton_mtx[game->frame_counter & 1],
                      aNogTriClock01_DwBefore, aNogTriClock01_DwAfter, ftr_actor);
}
...
static void aNogTriClock01_ct(FTR_ACTOR* ftr_actor, u8* data) {
    ...
    cKF_SkeletonInfo_R_ct(keyf, &cKF_bs_r_int_nog_tri_clock01, &cKF_ba_r_int_nog_tri_clock01, ftr_actor->joint,
                          ftr_actor->morph);
...
static aFTR_vtable_c aNogTriClock01_func = {
    &aNogTriClock01_ct, &aNogTriClock01_mv, &aNogTriClock01_dw, &aNogTriClock01_dt, NULL,
};
