...
static int aKonRedclock_DwAfter(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                void* arg, s_xyz* joint1, xyz_t* trans) {
    return 1;
}

static int aKonRedclock_DwBefore(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                 void* arg, s_xyz* joint1, xyz_t* trans) {
...
static void aKonRedclock_ct(FTR_ACTOR* ftr_actor, u8* data) {
    ...
    cKF_SkeletonInfo_R_ct(keyf, &cKF_bs_r_int_kon_redclock, &cKF_ba_r_int_kon_redclock, ftr_actor->joint,
                          ftr_actor->morph);
...
static aFTR_vtable_c aKonRedclock_func = {
    &aKonRedclock_ct, &aKonRedclock_mv, &aKonRedclock_dw, &aKonRedclock_dt, NULL,
};
