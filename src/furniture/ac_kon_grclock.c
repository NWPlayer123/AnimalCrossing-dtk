...
static int aKonGrclock_DwAfter(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                               void* arg, s_xyz* joint1, xyz_t* trans) {
    return 1;
}

static int aKonGrclock_DwBefore(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                void* arg, s_xyz* joint1, xyz_t* trans) {
...
static void aKonGrclock_ct(FTR_ACTOR* ftr_actor, u8* data) {
    ...
    cKF_SkeletonInfo_R_ct(keyf, &cKF_bs_r_int_kon_grclock, &cKF_ba_r_int_kon_grclock, ftr_actor->joint,
                          ftr_actor->morph);
...
static aFTR_vtable_c aKonGrclock_func = {
    &aKonGrclock_ct, &aKonGrclock_mv, &aKonGrclock_dw, &aKonGrclock_dt, NULL,
};
