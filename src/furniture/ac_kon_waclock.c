...
static int aKonWaclock_DwAfter(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                               void* arg, s_xyz* joint1, xyz_t* trans) {
    return 1;
}

static int aKonWaclock_DwBefore(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                void* arg, s_xyz* joint1, xyz_t* trans) {
...
static void aKonWaclock_ct(FTR_ACTOR* ftr_actor, u8* data) {
    ...
    cKF_SkeletonInfo_R_ct(keyf, &cKF_bs_r_int_kon_waclock, &cKF_ba_r_int_kon_waclock, ftr_actor->joint,
                          ftr_actor->morph);
...
static aFTR_vtable_c aKonWaclock_func = {
    &aKonWaclock_ct, &aKonWaclock_mv, &aKonWaclock_dw, &aKonWaclock_dt, NULL,
};
