...
static int aKonAmeclock_DwAfter(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                void* arg, s_xyz* joint1, xyz_t* trans) {
    return 1;
}

static int aKonAmeclock_DwBefore(GAME* game, cKF_SkeletonInfo_R_c* keyf, int jointNum, Gfx** joint, u8* jointFlag,
                                 void* arg, s_xyz* joint1, xyz_t* trans) {
...
static void aKonAmeclock_dw(FTR_ACTOR* ftr_actor, ACTOR* my_room_actor, GAME* game, u8* data) {
    cKF_Si3_draw_R_SV(game, &ftr_actor->keyframe, ftr_actor->skeleton_mtx[game->frame_counter & 1],
                      aKonAmeclock_DwBefore, aKonAmeclock_DwAfter, ftr_actor);
}
...
static void aKonAmeclock_ct(FTR_ACTOR* ftr_actor, u8* data) {
    ...
    cKF_SkeletonInfo_R_ct(keyf, &cKF_bs_r_int_kon_ameclock, &cKF_ba_r_int_kon_ameclock, ftr_actor->joint,
                          ftr_actor->morph);
...
static aFTR_vtable_c aKonAmeclock_func = {
    &aKonAmeclock_ct, &aKonAmeclock_mv, &aKonAmeclock_dw, &aKonAmeclock_dt, NULL,
};
