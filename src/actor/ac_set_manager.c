...
static int aSetMgr_renewal_player_next_pos(int* next_bx, int* next_bz) {
    ...
    wade_end_pos_res = get_player_actor_withoutCheck(play)->Get_WadeEndPos_proc(
        (GAME*)play, &endpos); // maybe make this a macro like GET_PLAYER_ACTOR() ?
...
static void aSetMgr_move(ACTOR* actor, GAME* game) {
    static aSetMgr_move_proc move[aSetMgr_MOVE_PROC_NUM] = { &aSetMgr_move_check_set, &aSetMgr_move_check_wait,
                                                             &aSetMgr_move_set };
...
ACTOR_PROFILE Set_Manager_Profile = { mAc_PROFILE_SET_MANAGER,
