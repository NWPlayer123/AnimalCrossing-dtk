...
ACTOR_PROFILE Groundhog_Control_Profile = { mAc_PROFILE_GROUNDHOG_CONTROL,
                                            ACTOR_PART_CONTROL,
...
static void aGHC_search_soncho(GAME_PLAY* play) {
    if (Common_Get(clip).groundhog_control_clip->attention_target_actor == NULL) {
        Common_Get(clip).groundhog_control_clip->attention_target_actor =
...
static void aGHC_before_800(GROUNDHOG_CONTROL_ACTOR* groundhog_control, GAME_PLAY* play) {
    if (aGHC_TIME_CHECK(8, 0, 0) && Common_Get(clip).event_manager_clip != NULL) {
        ...
        if ((*Common_Get(clip).event_manager_clip->title_fade_proc)(event_manager, mEv_EVENT_GROUNDHOG_DAY, 1,
                                                                    aEvMgr_EVENT_PLACE_SHRINE)) {
...
static void aGHC_birth_reset(GROUNDHOG_CONTROL_ACTOR* groundhog_control, GAME_PLAY* play) {
    ...
    if (Common_Get(clip).npc_clip != NULL && Common_Get(clip).npc_clip->setupActor_proc != NULL &&
        ...
        int spawned_actor =
            (*Common_Get(clip).npc_clip->setupActor_proc)(play, SP_NPC_EV_MAJIN, -1, -1, -1, bx, bz, 5, 8);
...
static void aGHC_setupAction(GROUNDHOG_CONTROL_ACTOR* groundhog_control, int action) {
    static aGHC_ACTION_PROC process[aGHC_ACTION_NUM] = { &aGHC_before_800,
