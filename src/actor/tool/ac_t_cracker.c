...
ACTOR_PROFILE T_Cracker_Profile = { mAc_PROFILE_T_CRACKER,
                                    ACTOR_PART_BG,
...
static void aTCR_actor_draw(ACTOR* actor, GAME* game) {
    ...
    if (cracker->tools_class.init_matrix == 1) {
        ...
        Matrix_translate(cracker->tools_class.actor_class.world.position.x,
                         cracker->tools_class.actor_class.world.position.y,
                         cracker->tools_class.actor_class.world.position.z, FALSE);
        Matrix_scale(0.01f, 0.01f, 0.01f, TRUE);
    }

    Matrix_scale(cracker->tools_class.actor_class.scale.x, cracker->tools_class.actor_class.scale.y,
                 cracker->tools_class.actor_class.scale.z, TRUE);
