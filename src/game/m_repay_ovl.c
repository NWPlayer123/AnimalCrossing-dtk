    ...
    if (total_money >= repay_ovl->repay_amount) {
        ...
        for (i = 0; i < MONEY_NUM; i++) {
            ...
            for (j = 0; j < mPr_POCKETS_SLOT_COUNT; j++) {
                if (mPr_GET_ITEM_COND(Common_Get(now_private)->inventory.item_conditions, j) == mPr_ITEM_COND_NORMAL &&
                    *item_p == *money_name_p) {
...
static void mRP_move_Play(Submenu* submenu, mSM_MenuInfo_c* menu_info) {
    static u32 step_money[mRP_CURSOR_NUM] = { 100000, 10000, 1000, 100, 10, 1, 0 };
...
static void mRP_repay_ovl_move(Submenu* submenu) {
    static mRP_MOVE_PROC ovl_move_proc[mSM_OVL_PROC_NUM] = { &mRP_move_Move, &mRP_move_Play, (mRP_MOVE_PROC)none_proc1,
                                                             (mRP_MOVE_PROC)none_proc1, &mRP_move_End };
...
static void mRP_set_num_str(GAME* game, u32 num, rgba_t* str_col_p, f32 x, f32 y, f32 scale) {
    ...
    mFont_SetLineStrings(game, str, sizeof(str), x, y, str_col_p->r, str_col_p->g, str_col_p->b, 255, FALSE, TRUE,
...
static void mRP_set_character_dl(Submenu* submenu, GAME* game, f32 x, f32 y) {
    ...
    mFont_SetLineStrings(game, kingaku_str, sizeof(kingaku_str), x + 153.0f, -y + 64.0f, 255, 255, 255, 255, FALSE,
                         TRUE, 0.875f, 0.875f, mFont_MODE_POLY);
    ...
    mFont_SetLineStrings(game, kettei_str, sizeof(kettei_str), x + 207.0f, -y + 128.0f, color_p->r, color_p->g,
                         color_p->b, 255, FALSE, TRUE, 0.875f, 0.875f, mFont_MODE_POLY);
