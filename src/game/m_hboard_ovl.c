...
static void mHB_hboard_ovl_move(Submenu* submenu) {
    static mSM_MOVE_PROC ovl_move_proc[mSM_OVL_PROC_NUM] = { &mHB_move_Move, (mSM_MOVE_PROC)&none_proc1, &mHB_move_Wait,
                                                             (mSM_MOVE_PROC)&none_proc1, &mHB_move_End };
    ...
    for (i = 0; i < mHB_LINE_NUM; i++) {
        ...
        do {
            ...
            if (line_check == mHB_LINE_CHECK_OVER_STR_LEN) {
                if (i != (mHB_LINE_NUM - 1) && ((input_str != str && input_str[-1] == CHAR_NEW_LINE) ||
                                                width + mFont_GetCodeWidth(*input_str, TRUE) > mHB_LINE_WIDTH_MAX)) {
                ...
                if (line_len != 0) {
                    mFont_SetLineStrings(game, str, line_len, x, y, 30, 0, 0, 255, FALSE, TRUE, 1.0f, 1.0f,
                                         mFont_MODE_POLY);
                }
        ...
        if (line_len != 0) {
            mFont_SetLineStrings(game, str, line_len, x, y, 30, 0, 0, 255, FALSE, TRUE, 1.0f, 1.0f, mFont_MODE_POLY);
        }
...
static void mHB_set_dl_sub(Submenu* submenu, GRAPH* graph, GAME* game, f32 x, f32 y) {
    ...
    if (editor_ovl != NULL) {
        /* Draw editor cursor */
        (*submenu->overlay->editor_ovl->cursol_draw)(submenu, game, x + editor_ovl->_26 + -7.0f,
...
static void mHB_set_dl(Submenu* submenu, GRAPH* graph, GAME* game, mSM_MenuInfo_c* menu_info) {
    ...
    /* Draw haniwa */
    (*submenu->overlay->hanwiaPortrait_ovl->set_haniwaPortrait_proc)(submenu, menu_info, graph, game, x + 230.0f,
                                                                     -y + 6.0f);
...
static void mHB_hboard_ovl_init(Submenu* submenu) {
    ...
    mSM_open_submenu_new2(submenu, mSM_OVL_EDITOR, mED_TYPE_HBOARD, 32,
                          Save_Get(homes[menu_info->data1].haniwa.message), mHB_LINE_WIDTH_MAX);
}
