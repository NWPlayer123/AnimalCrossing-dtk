...
static ACTOR* mBI_search_bg_item(Actor_info* info) {
    static const s16 name_data[] = { mAc_PROFILE_BGCHERRYITEM, mAc_PROFILE_BGITEM, mAc_PROFILE_BGWINTERITEM,
...
static void mBI_make_bg_item(GAME_PLAY* play) {
    if (mBI_search_bg_item(&play->actor_info) == NULL) {
        Actor_info_make_actor(&play->actor_info, &play->game, Common_Get(bg_item_profile), 0.0f, 0.0f, 0.0f, 0, 0, 0,
                              -1, -1, -1, EMPTY_NO, -1, -1, -1);
