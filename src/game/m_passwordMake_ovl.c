...
static void mPM_passwordMake_ovl_move(Submenu* submenu) {
    static mSM_MOVE_PROC ovl_move_proc[mSM_OVL_PROC_NUM] = { &mPM_move_Move, (mSM_MOVE_PROC)none_proc1,
                                                             (mSM_MOVE_PROC)none_proc1, &mPM_move_Obey, &mPM_move_End };
...
static void mPM_set_frame_dl(Submenu* submenu, GRAPH* graph, mSM_MenuInfo_c* menu_info, int sel_idx, f32 x, f32 y) {
    static u8 waku_color[mPM_ENTRY_NUM][3] = { { 150, 50, 30 }, { 255, 255, 255 } };
    ...
    gDPSetPrimColor(gfx++, 0, 255, waku_color[sel_idx ^ 1][0], waku_color[sel_idx ^ 1][1], waku_color[sel_idx ^ 1][2],
                    255);
...
static void mPM_set_character(Submenu* submenu, GAME* game, int sel_idx, f32 x, f32 y) {
    static u8 moji_color[mPM_ENTRY_NUM][3] = { { 20, 0, 0 }, { 120, 100, 80 } };
    ...
    /* Draw title text */
    mFont_SetLineStrings(game, title_str, sizeof(title_str), 141.0f + x, 34.0f - y, 255, 255, 255, 255, FALSE, TRUE,
                         0.875f, 0.875f, mFont_MODE_POLY);

    text_x = 136.0f + x;

    /* Draw town name */
    mFont_SetLineStrings(game, passwordMake_ovl->town_name, passwordMake_ovl->lengths[mPM_ENTRY_TOWN], text_x,
                         68.0f - y, moji_color[sel_idx][0], moji_color[sel_idx][1], moji_color[sel_idx][2], 255, FALSE,
                         TRUE, 1.0f, 1.0f, mFont_MODE_POLY);

    /* Draw player name */
    mFont_SetLineStrings(game, passwordMake_ovl->player_name, passwordMake_ovl->lengths[mPM_ENTRY_PLAYER], text_x,
                         96.0f - y, moji_color[sel_idx ^ 1][0], moji_color[sel_idx ^ 1][1], moji_color[sel_idx ^ 1][2],
