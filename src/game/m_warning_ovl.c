...
static mWR_line_c wr_mailbox_line[] = { mWR_LINE(wr_You_cant_hold, 70.0f, 28.0f),
                                        mWR_LINE(wr_any_more_letters_Want_to, 40.0f, 24.0f),
                                        mWR_LINE(wr_throw_away_some_old_ones, 36.0f, 24.0f),
                                        mWR_LINE(wr_Yes, 94.0f, 24.0f), mWR_LINE(wr_No, 140.0f, 0.0f) };

static mWR_line_c wr_pr_leave_line[] = { mWR_LINE(wr_You_cant_pull_out_a_present, 44.0f, 40.0f),
                                         mWR_LINE(wr_when_your_item_screen_is_full, 40.0f, 24.0f) };

static mWR_line_c wr_money_line[] = { mWR_LINE(wr_You_cant_carry, 66.0f, 40.0f),
                                      mWR_LINE(wr_more_than_99999_Bells, 44.0f, 24.0f) };

static mWR_line_c wr_pr_food_line[] = { mWR_LINE(wr_You_cant_mail, 70.0f, 40.0f),
                                        mWR_LINE(wr_turnips_or_living_things, 42.0f, 24.0f) };

static mWR_line_c wr_pr_quest_line[] = { mWR_LINE(wr_You_cant_mail, 74.0f, 40.0f),
                                         mWR_LINE(wr_someone_elses_belongings, 38.0f, 24.0f) };

static mWR_line_c wr_present_mail_line[] = { mWR_LINE(wr_You_cant_mail, 74.0f, 40.0f),
                                             mWR_LINE(wr_a_wrapped_present, 62.0f, 24.0f) };

static mWR_line_c wr_write_line[] = { mWR_LINE(wr_You_cant_carry, 85.0f, 40.0f),
                                      mWR_LINE(wr_any_more_letters, 82.0f, 24.0f),
                                      mWR_LINE(wr_so_you_cant_write_a_new_one, 45.0f, 24.0f) };

static mWR_line_c wr_original_line[] = { mWR_LINE(wr_Your_item_screen_is_full, 54.0f, 40.0f),
                                         mWR_LINE(wr_so_you_cant_use_a_pattern, 48.0f, 24.0f) };

static mWR_line_c wr_pr_card_line[] = { mWR_LINE(wr_You_cant_mail, 60.0f, 40.0f),
                                        mWR_LINE(wr_an_exercise_card, 54.0f, 24.0f) };

static mWR_line_c wr_put_max_furniture_line[] = { mWR_LINE(wr_You_cant_put, 68.0f, 40.0f),
                                                  mWR_LINE(wr_any_more_items_out, 50.0f, 24.0f) };

static mWR_line_c wr_put_furniture_line[] = { mWR_LINE(wr_You_cant_put, 68.0f, 40.0f),
                                              mWR_LINE(wr_any_more_items_there, 46.0f, 24.0f) };

static mWR_line_c wr_lock_diary_line[] = { mWR_LINE(wr_This_diary_is_locked, 38.0f, 46.0f) };

static mWR_line_c wr_music_line[] = { mWR_LINE(wr_You_cant_pull_anything_out, 47.0f, 40.0f),
                                      mWR_LINE(wr_when_your_item_screen_is_full, 41.0f, 24.0f) };

static mWR_line_c wr_music2_line[] = { mWR_LINE(wr_Theres_already_music_in_there, 38.0f, 46.0f) };

static mWR_line_c wr_hukubukuro_open_line[] = { mWR_LINE(wr_You_cant_open_a_bag, 48.0f, 37.0f),
                                                mWR_LINE(wr_unless_you_have_room, 50.0f, 24.0f),
                                                mWR_LINE(wr_for_three_more_items, 52.0f, 24.0f) };

static mWR_line_c wr_put_plant_line[] = { mWR_LINE(wr_You_cant_plant_anything_here, 39.0f, 46.0f) };

static mWR_line_c wr_put_sign_line[] = { mWR_LINE(wr_You_cant_put_a_sign_up_here, 40.0f, 46.0f) };

static mWR_line_c wr_put_sign_other_line[] = { mWR_LINE(wr_You_can_put_signs_up_only, 44.0f, 40.0f),
                                               mWR_LINE(wr_in_your_own_village, 64.0f, 24.0f) };

static mWR_line_c wr_put_sign_island_line[] = { mWR_LINE(wr_You_cant_put_signs_on_the_island, 31.0f, 46.0f) };

static mWR_line_c wr_put_fami_line[] = { mWR_LINE(wr_You_cant_leave, 72.0f, 40.0f),
                                         mWR_LINE(wr_this_NES_on_the_island, 44.0f, 24.0f) };

static mWR_line_c wr_word_over_line[] = { mWR_LINE(wr_You_dont_have_room, 54.0f, 40.0f),
                                          mWR_LINE(wr_to_write_any_more, 63.0f, 24.0f) };

static mWR_line_c wr_put_item_line[] = { mWR_LINE(wr_You_cant_drop, 72.0f, 40.0f),
                                         mWR_LINE(wr_any_more_items_here, 54.0f, 24.0f) };

static mWR_line_c wr_pw_chk_line[] = { mWR_LINE(wr_You_entered_letters, 59.0f, 40.0f),
                                       mWR_LINE(wr_that_have_nothing_to_do, 45.0f, 24.0f),
                                       mWR_LINE(wr_with_secret_codes, 63.0f, 24.0f) };

static mWR_line_c wr_pr_fork_line[] = { mWR_LINE(wr_You_cant_mail, 60.0f, 40.0f),
...
static mWR_data_c wr_win_data[] = { mWR_DATA(wr_mailbox_line, 1.0f, 1.2f),
...
static void mWR_warning_ovl_move(Submenu* submenu) {
    static mWR_MOVE_PROC ovl_move_proc[mSM_OVL_PROC_NUM] = { &mWR_move_Move, &mWR_move_Play, (mWR_MOVE_PROC)&none_proc1,
                                                             (mWR_MOVE_PROC)&none_proc1, &mWR_move_End };
...
static void mWR_set_frame_dl(Submenu* submenu, GRAPH* graph, mSM_MenuInfo_c* menu_info, mWR_data_c* data) {
    ...
    if (menu_info->data0 == 0) {
        ...
        y = (menu_info->position[1] + 60.0f * data->scale_y) -
            (data->lines[0].pos_y + data->lines[1].pos_y + data->lines[2].pos_y + data->lines[3].pos_y + 8.0f);
...
static void mWR_set_strings(Submenu* submenu, GAME* game, mSM_MenuInfo_c* menu_info, mWR_data_c* data) {
    ...
    for (i = 0; i < data->num_lines; i++) {
        ...
        mFont_SetLineStrings(game, line->str, line->str_len, x + line_x, y, color[0], color[1], color[2], 255, FALSE,
                             TRUE, scale, scale, mFont_MODE_POLY);
