...
void Ef_Room_Sunshine_Museum_actor_ct(ACTOR* actor, GAME* game) {
    ...
    museum->ef_sunshine_class.actor_class.world.position.y =
        mCoBG_GetBgY_OnlyCenter_FromWpos(museum->ef_sunshine_class.actor_class.world.position, 0.0f);
    ...
    switch (museum->ef_sunshine_class.actor_class.actor_specific) {
        ...
        case 2:
            ...
            museum->ef_sunshine_class.actor_class.world.position.y =
                1.0f + mCoBG_GetBgY_OnlyCenter_FromWpos(museum->ef_sunshine_class.actor_class.world.position, 0.0f) -
                40.0f;
        ...
        case 3:
            ...
            museum->ef_sunshine_class.actor_class.world.position.y =
                1.0f +
                mCoBG_GetBgY_OnlyCenter_FromWpos(pos3 = museum->ef_sunshine_class.actor_class.world.position, 0.0f) -
                40.0f;
            museum->ef_sunshine_class.actor_class.world.position.x += 6.0f;
            break;
    }
...
void Ef_Room_Sunshine_MuseumL_actor_move(ACTOR* actor, GAME* game) {
    ...
    if (Common_Get(time.now_sec) < 14400) {
        ...
        museum->ef_sunshine_class.actor_class.scale.x =
            calc_scale_Ef_Room_Sunshine_Museum(0, Common_Get(time.now_sec) - 43200);
    } else {
...
void Ef_Room_Sunshine_MuseumR_actor_move(ACTOR* actor, GAME* game) {
    ...
    if ((Common_Get(time.now_sec) >= 14400) && (Common_Get(time.now_sec) < 43200)) {
        museum->ef_sunshine_class.actor_class.scale.x =
            -calc_scale_Ef_Room_Sunshine_Museum(0, 43200 - Common_Get(time.now_sec));
    } else if (Common_Get(time.now_sec) >= 72000) {
        museum->ef_sunshine_class.actor_class.scale.x =
            -calc_scale_Ef_Room_Sunshine_Museum(1, 86400 - Common_Get(time.now_sec));
    } else {
        museum->ef_sunshine_class.actor_class.scale.x = 0.0f;
    }
}
...
void setup_mode_Ef_Room_Sunshine_Museum(ACTOR* actor, GAME_PLAY* play) {
    ...
    Matrix_translate(sunshine->actor_class.world.position.x + GETREG(TAKREG, 20),
                     sunshine->actor_class.world.position.y + GETREG(TAKREG, 21),
