...
static int lbRk_SeirekiDays(int year, int month) {
    // Table containing the number of days for each month in standard and leap years in the Seiyo Reki (Gregorian
    // calendar)
...
static int l_lbRk_leapdays[lbRk_YEAR_NUM] = { 0,  29, 0, 0,  29, 0, 29, 0, 0,  29, 0, 0,  30, 0, 29, 0, 0,
                                              29, 0,  0, 29, 0,  0, 30, 0, 29, 0,  0, 29, 0,  0, 30, 0 };
...
extern int lbRk_ToKyuuReki(lbRTC_ymd_c* kyuu_ymd, const lbRTC_ymd_c* seiyo_ymd) {
    ...
    while (TRUE) {
        ...
        if (seyio_date.day == seiyo_ymd->day && seyio_date.month == seiyo_ymd->month &&
            seyio_date.year == seiyo_ymd->year) {
        ...
        // If the kyuureki day exceeds the number of days in the current kyuureki month, reset the day and increment the
        // month
...
extern void lbRk_HarvestMoonDay(lbRTC_ymd_c* harvest_moon_day, int year) {
    ...
    static lbRk_date_t ev_day[lbRk_HARVEST_MOON_YEAR_NUM] = { /* 2002 */ { 9, 21 },
                                                              /* 2003 */ { 9, 10 },
                                                              /* 2004 */ { 9, 28 },
                                                              /* 2005 */ { 9, 18 },
                                                              /* 2006 */ { 10, 7 },
