...
static u32 F72(u32 crc, u32 src, vu8* keyp) {
    int i;                                                                                         // r31
    int poly = (keyp[0x13] << 8) + ((keyp[0x15] + (keyp[0x18] - (keyp[0x18] << 4))) - keyp[0x10]); // r30
...
static u32 F95(u32 src, vu8* keyp) {
    src = (src * ((keyp[3] << keyp[0x16]) | ((keyp[1] | (keyp[4] << keyp[0x11])) | (keyp[4] << keyp[0x18])))) -
          (keyp[7] - keyp[6]);
    return src;
}
...
s32 GBAJoyBootAsync(s32 chan, s32 paletteColor, s32 paletteSpeed, u8* programp, s32 length, u8* status,
                    GBACallback callback) {
    int ret;
...
void __GBAX01(s32 chan, s32 ret) {
    ...
    if (ret == GBA_READY) {
        ...
        if (bootInfo->readbuf[3] == 0 || bootInfo->readbuf[2] == 0 || (bootInfo->keyA & (D54[5] << 9)) == 0 ||
            bootInfo->readbuf[1] == 0 || (bootInfo->keyA >> 15) == 0 || bootInfo->readbuf[0] == 0) {
            ret = GBA_JOYBOOT_UNKNOWN_STATE;
        } else {
    ...
    if (ret == GBA_READY) {
        if (bootInfo->firstXfer != FALSE) {
            bootInfo->firstXfer = FALSE;
                   ...
                   (*bootInfo->status & D54[42]) >> D54[33] != (bootInfo->curOffset & D54[33]) >> D54[31]) {
            ret = GBA_JOYBOOT_UNKNOWN_STATE;
            goto exit;
        } else {
...
s32 GBAJoyBoot(s32 chan, s32 paletteColor, s32 paletteSpeed, u8* programp, s32 length, u8* status) {
    s32 ret = GBAJoyBootAsync(chan, paletteColor, paletteSpeed, programp, length, status, __GBASyncCallback);
    if (ret != GBA_READY) {
        return ret;
    }
